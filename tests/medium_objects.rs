...
fn main() {
	unsafe {
		...
		replace_nth(&mut objs, 2, Layout::from_size_align(2000, 32).unwrap());
		replace_nth(&mut objs, 3, Layout::from_size_align(1500, 16).unwrap());
		replace_nth(&mut objs, 5, Layout::from_size_align(1000, 8).unwrap());
